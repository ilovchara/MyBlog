...
下列程序通过查询不同程序的**DLL文件**，DLL文件是：Dynamic Link Library（动态链接库）文件的缩写，**作用是让程序的内存占用更小**。与静态链接库不同，动态链接库可以在程序运行时动态加载。一般而言，只有类库才有DLL文件，DLL 文件主要是类库的产物，控制台程序通常没有可供引用的 DLL。
...
**标准高的类库可以引用低版本的类库**，反之则不行。同时随着标准的变高，`NETCore`  和`NETFramework`也会随着需求变高，具体的标准如下，在开源项目中最好使用低版本的标准。
...
可以使用` Visual Studio`，但是无法跨平台使用。这里使用`vscode`配置`c#`文件。在一个简单的`c#`程序中，会自然生成几个文件夹。当然，在创建程序时不会有文件夹出现。
...
而转为`vscode`开发，其可以跨平台开发，但是需要注意的是其只能执行轻量级任务，在插件中下载`c#`以及`c# Dev Kit`即可

![image-20251113230733427](08-16-NET知识汇总/image-20251113230733427.png)

在`vscode`中只需要按住`Ctrl + Shift + N`即可唤出控制台，在控制台中选中需要创建的项目即可。

## NuGet共享代码库

`nuget`就是`.NET`的包管理器。相当于`npm`之于`node`、`pip`之于`Python`。

要使用`NuGet`程序包，在`Visual Studio`中打开项目，选择**工具菜单**下的`NuGet`包管理器，在弹出的窗口中搜索想要的程序包，点击**安装按钮**即可。`NuGet`会自动下载并引用程序包，并更新项目的配置文件。

此外可以通过访问网站的方式，来进行包的引入。以`DI`包作为例子。
...
这些都是上文中提到的一些概念，因为不知道怎么分类就归类到这里

这里算是分割线----------------------------------------------------------------------

## [MSBuild](https://www.cnblogs.com/shanyou/p/3452938.html?utm_source=chatgpt.com)

> 这里大部分都是抄写链接中的内容，可以直接看链接

`Microsoft Build Engine`，用于**控制生成平台处理和生成软件的方式**。通过在项目或解决方案文件中调用 `msbuild.exe` 或 `dotnet` 生成，可以在未安装` Visual Studio `的环境中协调和生成产品。它的核心作用是**根据项目文件**（如 `.csproj`, `.vbproj`, `.sln` 等）中的定义来执行**构建流程**。

### [.csproj文件](https://blog.walterlv.com/post/understand-the-csproj.html)

`.csproj`文件也就是项目文件，其中包含了可阅读的信息。每一种属性标签都会被`XML`标签包含。一般来说，一个项目文件的文件结构如下：

![传统的 csproj 文件格式](./08-16-NET知识汇总/2018-05-05-12-31-53.png)

在更新迭代之后，文件结构进行了简化，新版本的配置文件如下：

![跨平台的 csproj 文件格式](./08-16-NET知识汇总/2018-05-07-08-41-22.png)

在新格式中只有 `Project 节点` 和 `PropertyGroup` 是必要的。这些标签的作用不同，下面简单介绍一下：

- **PropertyGroup** ：其是属性的容器，它本身不直接影响构建，但承载的属性元素会生效。
- **ItemGroup**：其是属性的容器，不包含语义。
- **Import**：有两个。前面是为了导入属性（`props`），后面是为了导入 `Targets`.
- **Target**：其封装了`Task`，通过依赖关系参与整个构造流程
- **Project**：所有文件以其作为根节点，引入SDK

回到叙述[此文件的作用](https://www.cnblogs.com/shong/archive/2010/08/17/1801632.html)上，`Visual Studio`会根据`csproj`里的`XML`定义来管理项目文件以及相关其他一些种类非常丰富的数据及操作，`MSBuild`也会根据`csproj`文件来得知编译这个项目需要有哪些依赖，默认输出路径，`Pre-Build`和`Post-Build`需要哪些操作等等。
