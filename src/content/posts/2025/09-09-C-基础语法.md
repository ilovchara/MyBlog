...
引用类型和值类型，他们的区别就在于存储在的抽象数据结构的不同。值类型存储在**栈**中，引用类型存储在**堆**中。栈是一种先进后出的数据结构，堆是一种完全二叉树。讨论这二者类型，需要从内存入手：
...
而至于在`C#`中，堆栈对操作系统和内存的调度，由于本人水平有限，可以看这篇[文章](https://endjin.com/blog/2022/07/understanding-the-stack-and-heap-in-csharp-dotnet?utm_source=chatgpt.com).
