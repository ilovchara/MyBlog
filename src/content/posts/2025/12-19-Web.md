...
当 `var` 变量在函数外部声明时，在全局中所有语句都可以调用这个`var`变量。`var` 在函数中声明时，它的作用域是在函数体内。它只能在该函数中被访问。也就是说，子作用域可以调用父作用域的变量，但是父作用域无法调用子作用域的变量。
...
`var`有一个特点，其声明的变量可以重新声明和更新，在同一个作用域中这种操作不会导致报错。
...
**提升**是一种 JavaScript 机制，其中变量和函数声明在代码执行之前被移动到其作用域的顶部。这意味着，如果我们这样做：
...
```javascript
    var greeter; // 先声明 - 这样就不会报错了
    console.log(greeter); // greeter is undefined
    greeter = "say hello"
```

因此 `var` 变量被提升到其作用域的顶部，并使用 `undefined` 值进行初始化。但是使用`var`可能会导致变量覆盖问题，来看下面例子：
...
由于 `times > 3` 返回 `true`，变量在子作用域被赋值，`greeter` 被重新定义为 `"say Hello instead"`。也就是说对于var变量，它很容易被赋值因此使用这个变量风险很大。

>  **Let**

现在一般使用`let` ，它解决了我们刚刚介绍的 `var` 的问题。

块是由 `{}` 界定的代码块。一个块存在于花括号中。花括号内的任何内容都是一个块。而对于`Let`变量，**其声明只能在块级作用域才能生效**，可以重新赋值但是不能重复声明。
...
与`var`的区别就在于，其作用域被缩小了，如果跨了一个块就无法互相访问变量的信息。同时，其存在一个暂时性死区，就是当没有运行到声明的代码时，这个声明的变量存在但是不可访问。

> **const**

`const`声明用于声明块作用域的局部变量，通常被称之为常量，其值无法通过重新赋值来更改。其中特别的是，`const`也存在暂时性死区。
...
`const` 意味着你以后不能更改它。 `let` 意味着它可以被更改。它只存在于最近的一对花括号之间。 `var` 意味着可以在文件中的任何地方访问它。用 const 来定义我知道是常量的值。用`let` 来定义任何我需要重新赋值的东西。至于`var`少用。
...
数组是同类型变量的数据结构，通过索引来访问数组中的每一个元素，在`js`中声明一个数组的代码如下：
...
使用`.`操作符调用`length`可以输出调用数组的长度，通过赋值操作可以改变长度的值会改变当前数组大小。如果赋值操作的索引位置超过了原本数组的长度，则当前数组的长度会变长。
...
// 如果调用7位置 进行赋值操作 那么这个数组会变长
arr[8] = 10;
console.log(arr.length); // 变成8
```
...
JavaScript的对象是一种无序的集合数据类型，它由若干键值对组成。例如，为了描述小明，可以用若干键值对来描述他：
...
JavaScript用一个`{...}`表示一个对象，键值对以`xxx: xxx`形式申明，用`,`隔开。需要注意的是，最后一个键值对不需要在末尾加`,`，如果加了，有的浏览器（如低版本的IE）将报错。
...
实际上JavaScript对象的所有属性都是字符串，不过属性对应的值可以是任意数据类型。如果访问不存在的属性不报错，而是返回`undefined`：
...
由于JavaScript的对象是**动态类型**，可以自由地给一个对象添加或删除属性
...
但是如果`in`判断一个属性存在，这个属性不一定是`xiaoming`的，它可能是`xiaoming`继承得到的：
...
![js-proto](./12-19-Web/prototype-chain.jpg)
...
![js-proto-inherit](./12-19-Web/prototype-inherit.jpg)
